...
//        print (self.scrollView?.contentSize.height,self.scrollView?.contentOffset.y,self.scrollView?.contentInset.top,self.scrollView?.height,UIScreen.mainScreen().bounds.height)
        if self.height<=0{
            return
        }
        if (self.scrollView?.contentSize.height)!+(self.scrollView?.th_insetT)! > self.scrollView?.height{
